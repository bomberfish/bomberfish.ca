export class ProjectCardDetails {
	...
	/**
	 * Returns the path to the low-resolution thumbnail in /project-thumbnails.
	 * Returns undefined if no asset is associated with this project.
	 */
	getThumbnailPath(): string | undefined {
		if (!this.assetName) return undefined;
		return `/project-thumbnails/${this.assetName}`;
	}

	/**
	 * Returns the path to the high-resolution image in /project-images.
	 * Falls back to the thumbnail path if no high-res asset exists.
	 * Returns undefined if no asset is associated with this project.
	 */
	getImagePath(): string | undefined {
		if (!this.assetName) return undefined;
		if (!this.hasHighRes) return this.getThumbnailPath();
		return `/project-images/${this.assetName}`;
	}
}
